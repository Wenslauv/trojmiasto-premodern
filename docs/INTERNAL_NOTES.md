...
- `public/data/events/` — event JSON files and incoming edit/import files
- `public/data/templates/` — reusable starter templates for new data files
- `public/data/cache/` — autogenerated frontend cache files

## Data flow

1. Source of truth: `public/data/events.json`.
2. Event JSON files live in `public/data/events/` for import/export and tracking.
3. Template files live in `public/data/templates/` and are copied when creating new source files.
4. Optional derived cache generation: `npm run generate-data`.
5. Derived files are written to `public/data/cache` and can be committed together with new events.
...
```bash
cp public/data/templates/new-event.compact.template.json public/data/events/incoming-2026-08-10.compact.json
```
...
```bash
npm run add-event-compact -- --file public/data/events/incoming-2026-08-10.compact.json --dry-run
npm run add-event-compact -- --file public/data/events/incoming-2026-08-10.compact.json
```
...
```bash
cp public/data/templates/new-event.template.json public/data/events/incoming-2026-07-15.json
```

2. Validate and append event into source list:

```bash
npm run add-event -- --file public/data/events/incoming-2026-07-15.json
```
...
```bash
npm run add-event -- --file public/data/events/incoming-2026-07-15.json --dry-run
```

If you want to remove incoming file automatically after successful append:

```bash
npm run add-event -- --file public/data/events/incoming-2026-07-15.json --delete-source
```
...
```bash
npm run export-event -- --id 445001 --out public/data/events/edit-445001.json
```

2. Edit file `public/data/events/edit-445001.json`.

3. Validate update (dry run):

```bash
npm run update-event -- --id 445001 --file public/data/events/edit-445001.json --dry-run
```

4. Apply update:

```bash
npm run update-event -- --id 445001 --file public/data/events/edit-445001.json --delete-source
```
